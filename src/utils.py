...
def set_dataset_values():
    src.datasets.feature_num = 16
    src.datasets.label_num = 2

def arrays_to_tensor_stack(arrays):
    if (type(arrays) != 'numpy.ndarray'):
        print("hmm", arrays)
    # Convert each array into a PyTorch tensor
    tensors = [torch.tensor(arr) for arr in arrays]

    # Stack the tensors along a new dimension (0) to create a tensor stack
    stacked_tensor = torch.stack(tensors, dim=0)

    return stacked_tensor

def prepare_datasets(name: str = None):
    path = src.config.path_config.datasets + name;
    if name is not None:
        tr_features = pd.read_csv(path + 'x_train.csv')
        tr_features = torch.tensor(tr_features.values, dtype=torch.float)
        #values = arrays_to_tensor_stack(tr_features.values)
        #rint(values)

        #tr_features = torch.stack(torch.tensor(tr_features.values, dtype=torch.float))
        src.datasets.tr_samples = tr_features

        tr_labels = pd.read_csv(path + 'y_train.csv')
